...
export function buildSquads(standings = [], teams = []) {
  ...
  standings.forEach((m, mi) => {
    ...
    // 12th Man (booster): 1 cầu thủ thêm, ngoài đội chính & dự bị, không tính ngân sách.
    // Sinh sẵn cho mọi manager (deterministic); chỉ hiển thị khi manager dùng chip này.
    const twelfthMan = pick(["FWD", "MID", "DEF"][seed0 % 3], 1)[0] || null;
    ...
    squads[m.manager] = { formation: "5-3-2", starters, bench, twelfthMan, squadValue: Number(squadValue.toFixed(1)) };
  });
