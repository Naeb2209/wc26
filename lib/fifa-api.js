...
function mergeFantasyRanks(liveRanks, savedRanks) {
  const liveByUserId = new Map(
    (liveRanks || []).filter((rank) => rank.userId != null).map((rank) => [Number(rank.userId), rank])
  );
  const liveByManager = new Map((liveRanks || []).map((rank) => [rank.manager, rank]));
  const savedUserIds = new Set(
    (savedRanks || []).filter((rank) => rank.userId != null).map((rank) => Number(rank.userId))
  );

  const mergedSaved = (savedRanks || []).map((saved) => {
    const live = liveByUserId.get(Number(saved.userId)) || liveByManager.get(saved.manager) || {};
    return { ...live, ...saved };
  });
  const newLive = (liveRanks || []).filter(
    (live) => live.userId == null || !savedUserIds.has(Number(live.userId))
  );

  return [...mergedSaved, ...newLive];
}

export async function getFantasy() {
  const db = await readDb();
  let base = db.fantasy || { leagueName: "", updatedRound: "", standings: [] };

  // Khi đã cấu hình KV: readDb() đã trả về fantasy mới nhất từ KV -> khỏi cần raw GitHub.
  // Chưa có KV (chế độ cũ): production đọc thẳng snapshot đã commit trên GitHub để số liệu
  // tươi mà không phải chờ Vercel build lại sau mỗi lần sync.
  if (!kvConfigured() && process.env.VERCEL) {
    try {
      const snapshotRes = await fetch(
        `https://raw.githubusercontent.com/Naeb2209/wc26/main/data/db.json?t=${Date.now()}`,
        { cache: "no-store" }
      );
      if (snapshotRes.ok) {
        const snapshot = await snapshotRes.json();
        if (snapshot?.fantasy?.standings?.length) base = snapshot.fantasy;
      }
    } catch (error) {
      console.error("[fifa-api] remote fantasy snapshot fallback:", error.message);
    }
  }
