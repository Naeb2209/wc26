...
function annotateChannels(schedule, broadcast) {
  const vb = schedule?.["Vòng Bảng"]?.matches;
  if (vb) {
    for (const r of Object.keys(vb)) {
      for (const m of vb[r]) m.channel = resolveChannel(broadcast, m.homeCode, m.awayCode);
    }
  }
  return schedule;
}
...
export async function getSchedule() {
  ...
  for (const provider of providerOrder()) {
    try {
      ...
      if (total) {
        const schedule = annotateChannels({ "Vòng Bảng": groupData }, db.broadcast);
  ...
  return { stages: db.stages, schedule: annotateChannels(db.schedule, db.broadcast), source: "sample" };
}
