...
export async function getStandings() {
  ...
  for (const provider of providerOrder()) {
    try {
      ...
      if (groups.length) return { groups: sortGroupRows(groups), source: provider };
  ...
  return { groups: sortGroupRows(db.groups), source: "sample" };
}
...
export async function getSchedule() {
  ...
  for (const provider of providerOrder()) {
    try {
      ...
      if (total) {
        const schedule = annotateChannels({ "Vòng Bảng": groupData }, db.broadcast, db.fifaRanks);
        return { stages: db.stages, schedule, source: provider };
      }
  ...
  return { stages: db.stages, schedule: annotateChannels(db.schedule, db.broadcast, db.fifaRanks), source: "sample" };
}
...
export async function getFantasy() {
  ...
  try {
    ...
    return { ...base, standings: mapFantasyRanks(ranks), source: "fifa" };
  } catch (e) {
