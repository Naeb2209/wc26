...
// Tra cứu avatarPos {x,y,scale} đã cấu hình theo (mã đội, tên). Trả về null nếu không có.
// Dùng để chỉnh vị trí/zoom avatar trong khung thẻ Fantasy.
export function buildAvatarPosLookup(teams = []) {
  const byTeam = new Map();
  for (const t of teams || []) {
    const arr = [];
    for (const p of t.players || []) {
      if (!p.name || !p.avatarPos) continue;
      arr.push({ n: norm(p.name), tk: tokens(p.name), pos: p.avatarPos });
    }
    byTeam.set(t.code, arr);
  }
  ...
  return (code, name) => {
    ...
    const tk = tokens(name);
    const cand = arr.filter((p) => subset(tk, p.tk) || subset(p.tk, tk));
    return cand.length === 1 ? cand[0].pos : null;
  };
}
